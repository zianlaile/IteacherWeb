!function($, wysi) {
    ...
    var tpl = {
        ...
        "image": function(locale, options) {
            var size = (options && options.size) ? ' btn-'+options.size : '';
            return "<li>" +
              "<a class='btn btn-default insertImage" + size + "' title='" + locale.image.insert + "' tabindex='-1'><i class='fa fa-picture-o'></i></a>" +
            "</li>";
        },
    ...
    Wysihtml5.prototype = {
        ...
        initInsertImage: function(toolbar) {
        	var $file = $('<input type="file" id="file-' + new Date().getTime() + '"/>');
            toolbar.find('a.insertImage').append($file);
            var caretBookmark;
            $file.uploadify({
            	swf: ctx + '/assets/plugins/uploadify.swf',
        		uploader: ctx + '/notice/uploadimage',
        		fileObjName: 'image',
        		...
        		onUploadSuccess: function(file, data, response){
        			...
        			$.messager.progress('close');
        			if(!jdata.success){				
        				$.messager.alert('系统信息', '上传失败', 'error');
        				return;
        			}
        			var loc = document.location.href;
        			var protocal = loc.substring(0, loc.indexOf('://') + 3);
        			loc = loc.substring(protocal.length);
        			var domain = protocal + loc.substring(0,loc.indexOf('/'));       			
        			var url = domain + ctx +  jdata.data;
